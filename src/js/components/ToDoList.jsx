...
const ToDoList = ({shareUser}) => {
	const [taskList, setTaskList] = useState([]);

	const getToDos = () => {
		fetch("https://playground.4geeks.com/todo/users/" + shareUser)
			.then(response => {
				if (!response.ok)
	...
	const postToDo = async (toDo) => {
		try {
			const response = await fetch("https://playground.4geeks.com/todo/todos/" + shareUser, {
				method: "POST",
				body: JSON.stringify({
					"label": toDo,
					"is_done": false
